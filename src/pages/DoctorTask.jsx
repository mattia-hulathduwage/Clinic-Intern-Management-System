...
import DoctorTaskAction from "../components/DoctorTaskAction";
...
const DoctorTask = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const doctorId = localStorage.getItem("doctor_id"); // ✅ Moved inside function
        ...
        const urls = [
          `http://localhost:5000/api/labtasks?doctor_id=${doctorId}`,
          `http://localhost:5000/api/tasks/wardvisit?doctor_id=${doctorId}`,
          `http://localhost:5000/api/completelabtasks?doctor_id=${doctorId}`,
          `http://localhost:5000/api/tasks/completewardvisit?doctor_id=${doctorId}`,
        ];
  ...
  // Ensure it re-fetches when doctorId changes
  ...
  return (
    <div style={styles.container}>
      ...
      {isPopupVisible && (
        <DoctorTaskAction onClose={() => setIsPopupVisible(false)} />
      )}
    </div>
  );
};

export default DoctorTask;
